...
get_gravatar = function(email, size) {
    ...
    return 'http://www.gravatar.com/avatar/' + MD5(email) + '.jpg?s=' + size+"&d=blank";
};
...
isValide = function (val) {
    access = '';
    for (var i = 0; i <= (Meteor.users.find().count() -1); i++) {
      if ( val === Meteor.users.find().fetch()[i].profile.access) {
        access = 'admin';
      }
    }
    if (access === 'admin') {
      ...
      console.log('Você não é bem vindo aqui!...');
      return false;
    }
};
...
isNotEmpty = function (val) {
  ...
  if (!val || val === ''){
    humane.log('Por favor complete todos os campos.');
    return false;
  }
  return true;
};
...
//Function of 3h!!!
isNotTheSameMan = function (val) {
  var email = Meteor.users.findOne({'emails.address': {$regex:val,$options:'i'}});
  if ( email === undefined) {
    return true;
  } else {
    var emails = Meteor.users.findOne({'emails.address': {$regex:val,$options:'i'}}).emails;
    if (val === _.map(emails, function(obj) { return obj.address; })[0] ) {
      humane.log('Esse usuário já está cadastrado.');
      return false;
    }
  }
}
...
if (Meteor.user()) {
var user = Meteor.user();
emailSendTo = (user.emails && user.emails[0] && user.emails[0].address);
//
}
});
